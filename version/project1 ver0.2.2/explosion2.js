function Explosion2(x, y, r, maxRadius, colorpoint, count){
    this.x = x;
    this.y = y;
    this.r = r;
    this.minR = r;
    ...
    this.colorArr = [["rgba(255, 255, 0,"],//y
                     ["rgba(255, 153, 0,"]];//o
    this.colorPoint = colorpoint ? colorpoint : 0;

    this.dead = false;

    //----------------------
    this.clone = function(){
        if(this.dead && this.count < this.maxC){
            var explo = new Explosion2(this.x,this.y,this.maxR,this.maxR + 15, this.colorPoint, this.count);
            explo.count++;
            explo.colorPoint+=1;
